...
const chatFeed = queryRequired("#chat-feed");
const emptyState = queryRequired("#empty-state");
const sessionStatus = queryRequired("#session-status");
const newSessionBtn = queryRequired("#new-session-btn");
const modeButtons = Array.from(
  document.querySelectorAll(".toggle-btn")
);
const fileInput = queryRequired("#file-input");
const fileName = queryRequired("#file-name");
const messageInput = queryRequired("#message-input");
const clearInputBtn = queryRequired("#clear-input-btn");
const sendBtn = queryRequired("#send-btn");
...
function appendAssistantBody(container, text) {
...
function updateMessage(card, role, text, options = {}) {
  card.className = `message-card ${role}`;
...
function resetSession() {
  ...
  Array.from(chatFeed.querySelectorAll(".message-card")).forEach(
    (node) => node.remove()
  );
...
function buildRequestBody(message, attachment) {
  ...
  if (attachment) {
    ...
    return { body: formData, headers: void 0 };
  }
  const payload = {
    message
  };
...
async function sendMessage() {
  ...
  const message = rawInput || "Please analyze the attached log file and suggest the next checks.";
